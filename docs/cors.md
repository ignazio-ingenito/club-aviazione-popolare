...
- Browser → `https://www.clubaviazionepopolare.org` (Next.js)
...
1) **Direct link to Directus assets** (simpler)
   ...
   - Ensure Next Image allows that host (already updated in `next.config.mjs` for `cms.clubaviazionepopolare.org`).
...
- `CORS_ENABLED=true`
- `CORS_ORIGIN=https://www.clubaviazionepopolare.org`
