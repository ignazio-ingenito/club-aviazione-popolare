...
DIRECTUS_FEEDS_URL = urljoin(DIRECTUS_API_URL, "items/feeds")
...
def create_item(post: DirectusPost) -> DirectusPost:
    ...
    # `feeds.cover` on cap-cms is UUID-backed, so keep cover as-is.
    attempts: list[dict] = [
        data,
    ...
    for idx, payload in enumerate(attempts, start=1):
        try:
            res = client.post(DIRECTUS_FEEDS_URL, json=payload)
...
def find_item(field: str, value: str) -> Optional[str]:
    response = client.get(
        DIRECTUS_FEEDS_URL, params={f"filter[{field}][_eq]": value, "limit": 1}
    )
    response.raise_for_status()
    data = response.json().get("data", [])
    if not data:
